...
def type2widget(type_: Union[type]) -> Optional[Type[WidgetType]]:
    ...
    if Color is not None:  # skips this mapping if colour library not installed
        simple[Color: QColorDialog]
    if type_ in simple:
        return simple[type_]
...
def getter_setter_onchange(widget: WidgetType) -> GetSetOnChange:
    ...
    if isinstance(widget, QComboBox):
        ...
        def getter():
            red, green, blue, alpha = widget.currentColor().getRgb()
            color_value = Color(rgb=(red / 255, green / 255, blue / 255))
            color_value.set_saturation(alpha / 255)
            return color_value

        return GetSetOnChange(getter, widget.setCurrentColor, widget.colorSelected)
